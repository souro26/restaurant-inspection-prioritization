...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Score restaurants and create a capacity-constrained priority queue."
        )
    )
...
def score_restaurants(
    ...
    if tuple(X.columns) != artifacts.feature_names:
        raise ValueError(
            "Scoring feature schema does not match the frozen model artifact contract."
        )

    raw_scores = artifacts.model.predict_proba(X)[:, 1]

    calibrated_scores = artifacts.calibrator.predict_positive_probability(
    ...
    priority_queue = scored.head(capacity).copy()
...
def main() -> None:
    ...
    all_scores_path = config.output_directory / "restaurant_risk_scores.csv"

    priority_queue_path = config.output_directory / "restaurant_priority_queue.csv"
