...
def random_expected_metrics(data: pd.DataFrame, capacity: int) -> dict[str, float]:
    ...
    return {
        ...
        "expected_high_severity_outcomes": (capacity * prevalence),
        "expected_precision_at_k": prevalence,
        "lift_vs_random": 1.0,
    }


def evaluate_ranking_policy(
    data: pd.DataFrame, score_column: str, capacity: int, policy_name: str
) -> dict[str, float | str]:
    ...
    return {
        ...
        "lift_vs_random": (
            precision_at_k / prevalence if prevalence > 0 else float("nan")
        ),
    }
