...
def create_primary_temporal_splits(
    df: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Create the primary temporal split."""
    cohort = df[
        (df.cutoff_date >= "2022-01-01") & (df.cutoff_date < "2026-01-01")
    ].copy()
    train_df = cohort[cohort.cutoff_date < "2024-01-01"].copy()
    validation_df = cohort[
        (cohort["cutoff_date"] >= "2024-01-01") & (cohort["cutoff_date"] < "2025-01-01")
    ].copy()
    test_df = cohort[
        (cohort["cutoff_date"] >= "2025-01-01") & (cohort["cutoff_date"] < "2026-01-01")
    ].copy()
