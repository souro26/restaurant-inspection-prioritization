...
def fit_logistic_regression(
    train_df: pd.DataFrame, c: float = 1.0, max_iter: int = 1000, random_state: int = 42
) -> Pipeline:
...
def predict_logistic_probabilities(
    model: Pipeline, data_to_score: pd.DataFrame
) -> pd.Series:
    ...
    return pd.Series(
        ...
        name="logistic_high_severity_probability",
    )


def add_logistic_probabilities(
    model: Pipeline,
    data_to_score: pd.DataFrame,
    score_column: str = "logistic_high_severity_probability",
) -> pd.DataFrame:
    """Return a copy of data_to_score with logistic probabilities added."""
    result = data_to_score.copy()
    result[score_column] = predict_logistic_probabilities(
        model=model, data_to_score=result
    )
    return result
