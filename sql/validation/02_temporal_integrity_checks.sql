-- 02_temporal_integrity_checks.sql
-- Purpose: Validate the retrospective prediction population, labels,
--     and point-in-time temporal relationships.
...
SELECT
    ...
    *
FROM processed.labels
WHERE target_critical NOT IN (0, 1)
   OR target_critical IS NULL;
...
SELECT
    ...
    SUM(target_critical) AS critical_targets,
    ROUND(100.0 * AVG(target_critical), 2)
